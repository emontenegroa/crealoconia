...
-- Keep only safe insert policy for anonymous users (lead generation functionality)
-- But restrict it to prevent abuse
...
WITH CHECK (
  ...
  -- Prevent duplicate submissions within 1 hour per email
  AND NOT EXISTS (
    SELECT 1 FROM public.form_submissions 
    WHERE form_submissions.email = NEW.email 
    AND created_at > NOW() - INTERVAL '1 hour'
  )
);

-- Create a secure function for users to access their own submissions via email verification
CREATE OR REPLACE FUNCTION public.get_user_submission(user_email text, verification_code text DEFAULT NULL)
RETURNS TABLE(
  id uuid,
  form_data jsonb,
...
AS $$
BEGIN
  -- Simple email-based access (in production, you'd want proper verification)
  -- This allows users to view their own submissions by providing their email
...
VALUES (
  ...
  jsonb_build_object(
    ...
    'description', 'Removed dangerous public read/update policies'
  )
);